...
- **可拖曳浮動按鈕** — 不擋閱讀，位置自動記憶，磁吸左右邊緣，可在設定中隱藏
- **自訂翻譯樣式** — 文字顏色、背景色、字體大小皆可調整
- **設定即時生效** — 所有選項切換後自動儲存，無需手動按儲存按鈕
...
基於安全考量,Chrome **預設不允許**擴充功能存取本地檔案。因此用瀏覽器直接開啟本機的 HTML、PDF 等檔案時,「譯」不會生效——這不是故障,只需手動授權一次即可。

### Chrome / Edge / Brave

1. 開啟 `chrome://extensions`(Edge 為 `edge://extensions`)
2. 找到「譯」,點擊「詳細資料」
3. 開啟「**允許存取檔案網址(Allow access to file URLs)**」開關
4. 重新整理本地檔案頁面

授權後,全頁翻譯、劃詞翻譯與本地 PDF 翻譯(`PDF ➜`)都能正常運作。

### Firefox

Firefox 開啟本地檔案時通常可直接使用,無需額外設定。
